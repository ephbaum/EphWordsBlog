...
- Completed a major dependency update, upgrading to Astro v5, updating all packages to their latest versions, and migrating to modern tooling (ESLint 9, Husky 9, etc.).
- Migrated to pnpm as the primary package manager for improved performance and consistency.
- See `DEPENDENCY_UPDATE_SUMMARY.md` and `MIGRATION_TO_PNPM.md` for details.
...
- **[TypeScript](https://www.typescriptlang.org/)** v5.8.3 - Type-safe development with strict configuration
...
- **Shiki Syntax Highlighting** - Dual theme support (light/dark) with transformers

#### Build & Optimization

- **[Pagefind](https://pagefind.app/)** - Static search for fast, accurate content discovery
...
│   │   ├── Search.astro  # Pagefind search functionality
...
- **Static Search** - Fast, accurate search with Pagefind
...
- **Minimal JavaScript** - Astro-only components for faster loading
...
- Node.js 22.12.0+ (managed via asdf or nvm)
...
- `pnpm run build` - Build production site with Pagefind indexing
...
5. **Deployment** - Build and deploy with `pnpm run build`
...
2. **Astro Build** - Generates static HTML/CSS/JS from Astro components
3. **Pagefind Indexing** - Creates static search index
4. **Firebase Deploy** - Uploads to global CDN with automatic HTTPS
...
- **Bundle Size** - Minimal JavaScript footprint with Astro-only components
